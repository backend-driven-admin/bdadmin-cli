...
export default function validateConfig(config: any): string | undefined {
	...
	if (
		!config.auth.login.endpoint ||
		!config.auth.login.endpoint?.url ||
		!config.auth.login.endpoint?.method
	) {
	...
	if (
		!config.auth.login.fields ||
		!Array.isArray(config.auth.login.fields) ||
		config.auth.login.fields.length === 0
	) {
	...
	if (
		!config.auth.refresh.endpoint ||
		!config.auth.refresh.endpoint?.url ||
		!config.auth.refresh.endpoint?.method
	) {
	...
	if (
		!config.auth.logout.endpoint ||
		!config.auth.logout.endpoint?.url ||
		!config.auth.logout.endpoint?.method
	) {
